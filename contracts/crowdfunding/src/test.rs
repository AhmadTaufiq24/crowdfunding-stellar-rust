...
use super::*;
use soroban_sdk::{testutils::Address as _, testutils::Ledger, Address, Env, String}; // TAMBAHKAN String
// use ::token::{TokenContract, TokenContractClient}; // GUNAKAN ::token untuk menghindari ambigu
...
fn test_get_goal() {
    ...
    let deadline = env.ledger().timestamp() + 86400;
    let xlm_token = Address::generate(&env); // Tambahkan XLM token address

    env.mock_all_auths();

    client.initialize(&owner, &goal, &deadline, &xlm_token); // Tambahkan parameter ke-4
...
//     // 1. Deploy token contract terlebih dahulu
...
//     let supply = 1_000_000_000i128; // 1000 tokens
//     token_client.initialize(&owner, &name, &symbol, &supply);
    
//     // 4. Initialize crowdfunding contract dengan token contract address
//     client.initialize(&owner, &goal, &deadline, &token_contract_id);
...
fn test_is_ended() {
    ...
    let xlm_token_address = Address::from_string(&soroban_sdk::String::from_str(&env, XLM_CONTRACT_TESTNET)); // TAMBAHKAN INI

    env.mock_all_auths();

    client.initialize(&owner, &goal, &deadline, &xlm_token_address); // TAMBAHKAN &xlm_token_address
...
fn test_get_deadline() {
    ...
    let xlm_token = Address::generate(&env); // Tambahkan ini

    env.mock_all_auths();

    client.initialize(&owner, &goal, &deadline, &xlm_token); // Tambahkan &xlm_token

    // Test get_deadline returns correct value
    assert_eq!(client.get_deadline(), deadline);
}
// - test_get_progress_percentage() return correct percentage
// #[test]
...
// // - test_refund() return correct amount
// #[test]
